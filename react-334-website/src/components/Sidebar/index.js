...
const Sidebar = (props) => {
  ...
  const renderLinks = () => {
    ...
    if (pathname.includes("phar")) return LINKS.PHAR_PRESCRIPTION;
    return null;
  };
    ...
    <>
      {links && (
        <div className={cx(styles.container, styles[userInfo.role])}>
              ...
              >
                <span
                  className={cx(
                    styles.linkContent,
                    pathname === link.path
                      ? styles[`linkActive-${userInfo.role}`]
                      : null
                  )}
                >
                  {link.content}
          ...
          <div>
            {isShowUser && userInfo?.token && (
              <span
                className={cx(styles.link, styles[`name-${userInfo.role}`])}
              >
                {userInfo.fullname}
              </span>
            )}
